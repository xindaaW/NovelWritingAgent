...
class Config(BaseModel):
    ...
    def load(cls) -> "Config":
        ...
        if not config_path.exists():
            raise FileNotFoundError("Configuration file not found. Place config.yaml in config/ or copy from config/config-example.yaml.")
    ...
    def find_config_file(cls, filename: str) -> Path | None:
        ...
        1) config/{filename} in current directory (standalone project mode)
